...
bucket = storage_client.bucket("gcloud-training-hbkt3")
...
def save_model_artifact(model_name,pipeline):
    ...
    model_artifact = bucket.blob('ml_artifacts/'+artifact_name)
    model_artifact.upload_from_filename(artifact_name)

def load_model_artifact(file_name):
    blob = bucket.blob("ml_artifacts/" + file_name)
    blob.download_to_filename(file_name)
    return load(file_name)

def write_metrics_to_bigquery(algo_name, training_time, model_metrics):
    client = bigquery.Client()
    table_id = "gcloud-training-4135298-bs7.ml_ops.bank_campaign_model_metrics"
...
def main():
    input_data_path = "gs://gcloud-training-hbkt3/bank_campaign_data/bank-campaign-training-data.csv"
    model_name='xgboost'
